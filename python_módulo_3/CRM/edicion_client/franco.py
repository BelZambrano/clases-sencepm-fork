...
while True:
    try:
        ...
        cliente = next((c for c in clients if c["id"] == id_input), None)

        if not cliente:
            print("❌ Cliente no encontrado.")
            continue

        campos = [k for k in cliente.keys() if k != "id"]
        print(f"Campos disponibles: {', '.join(campos)}")
        campo = input("Selecciona campo a modificar: ").strip()

        if campo not in campos:
            print("❌ Campo inválido.")
            continue

        cliente[campo] = input(f"Ingrese nuevo valor para '{campo}': ")
        print(f"✅ Cliente actualizado: {cliente}")
    
    except ValueError:
        print("❌ Error: El ID debe ser un número.")

    if input("\n¿Deseas ingresar otra modificación? (s/n): ").lower() != "s":
        break
