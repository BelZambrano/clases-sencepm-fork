...
if s.isdigit():
    ...
    if obj is None:
        ...
        print("Actual:", obj)
        campo = input("Campo (nombre_completo/correo/telefono/estado): ").strip()
        if campo == "estado":
            print("1) Cliente potencial\n2) Alto interés\n3) Cliente efectivo\n4) En proceso de compra\n5) Super cliente")
            op = input("Selecciona 1-5: ").strip()
            if op in "12345" and len(op)==1:
                obj["estado"] = ["Cliente potencial","Alto interés","Cliente efectivo","En proceso de compra","Super cliente"][int(op)-1]
                print("Actualizado:", obj)
            else:
                print("Opción inválida")
        elif campo in obj and campo != "id":
            obj[campo] = input("Nuevo valor: ")
            print("Actualizado:", obj)
        else:
