...
while not funciono:
    try:
        id = int(input("Ingrese el ID del cliente que desea kickear:\n "))
        for cliente in clients:
            if cliente["id"] == id:
                kickeados.append(cliente)
                clients.remove(cliente)
                print("Cliente kickeado correctamente")
                funciono = True
                break
    except ValueError:
        print("ID inválido. Por favor, ingrese un ID valido") 
        continue
if funciono == False:
