...
# Ejemplo de edición directa
id_buscar = 3
campo_editar = "estado"
nuevo_valor = "Super cliente"

cliente_encontrado = False
for cliente in clients:
    if cliente["id"] == id_buscar:
        if campo_editar in cliente:
            cliente[campo_editar] = nuevo_valor
            print(f"Cliente {id_buscar} actualizado: {campo_editar} = {nuevo_valor}")
        else:
            print(f"Error: El campo '{campo_editar}' no existe")
        cliente_encontrado = True
        break

if not cliente_encontrado:
    print(f"Error: Cliente con ID {id_buscar} no encontrado")
