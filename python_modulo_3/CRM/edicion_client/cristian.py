...
print("=== SISTEMA DE EDICIÓN DE CLIENTES ===")


for c in clientes:
    print(f"ID: {c['id']} | Nombre: {c['nombre_completo']} | Teléfono: {c['telefono']} | Correo: {c['correo']} | Estado: {c['estado']}")
...
if cliente_encontrado:
    ...
    opcion = input("Seleccione el número del campo que desea editar: ")

    match opcion:
        case "1":
            nuevo_valor = input("Ingrese el nuevo nombre: ")
            cliente_encontrado["nombre_completo"] = nuevo_valor

        case "2":
            nuevo_valor = input("Ingrese el nuevo teléfono (+56XXXXXXXXX): ")
            if nuevo_valor.startswith("+56") and nuevo_valor[3:].isdigit():
                cliente_encontrado["telefono"] = nuevo_valor
            else:
                print("Teléfono inválido. No se realizaron cambios.")

        case "3":
            nuevo_valor = input("Ingrese el nuevo correo: ")
            if "@" in nuevo_valor and "." in nuevo_valor:
                cliente_encontrado["correo"] = nuevo_valor
            else:
                print("Correo inválido. No se realizaron cambios.")

        case "4":
            nuevo_valor = input("Ingrese el nuevo estado (cliente potencial / alto interés / en proceso de compra / cliente efectivo / super cliente): ")
            if nuevo_valor.strip() != "":
                cliente_encontrado["estado"] = nuevo_valor
            else:
