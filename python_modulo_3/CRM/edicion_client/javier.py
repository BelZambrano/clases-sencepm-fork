...
id_a_buscar = int(input("Ingresa el id del cliente que quieres editar: "))

cliente_encontrado = None

for cliente in clients:
    if cliente["id"] == id_a_buscar:
...
if cliente_encontrado is None:
    ...
    print("\nCampos disponibles: id, nombre_completo, correo, telefono, estado")
    campo = input("¿Qué campo quieres editar? (escribe exactamente el nombre de la clave): ")

    if campo in cliente_encontrado:
        nuevo_valor = input(f"Nuevo valor para '{campo}': ")

        if campo == "id":
            nuevo_valor = int(nuevo_valor)

        cliente_encontrado[campo] = nuevo_valor
        print("\nCliente actualizado:")
        print(cliente_encontrado)
    else:
        print("El campo que escribiste no existe en el cliente.")
